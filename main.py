...
# Initialize AWOA
awoa = AWOA(alpha=1.0, beta=0.9, gamma=0.001, update_interval=5)
n_majority, n_minority = np.sum(y_train == 0), np.sum(y_train == 1)
w0, w1 = awoa.initialize_weights(n_majority, n_minority)
...
for epoch in range(epochs):
    for i in range(0, len(x_train), batch_size):
        ...
        if epoch % awoa.update_interval == 0:
            bias = awoa.compute_bias(y_batch, y_pred)
            w0, w1 = awoa.update_weights(bias)
